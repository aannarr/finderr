...
| `FINDERR_SHELVES_HIDDEN_BY_DEFAULT` | | Comma list of shelf ids that start switched OFF for everybody who has not said otherwise — `coming-soon-series,genre-adventure`. A genre row's id is `genre-` plus its lowercased name. It is a default and not a removal: the shelf is still built, still pre-warmed, and still listed under **Your front page** on `/account` marked hidden, so anyone can turn it back on and their choice outranks this from then on. An id naming no current shelf is ignored, because the genre rows rotate with the nightly index build |
...
| `FINDERR_MEDIA_VOLUMES` | | **The whole playback switch, and it is an allow-list rather than a convenience.** `arrPath=localPath` pairs, comma separated: `/plex=/plex`, or `/plex=/Volumes/media,/archive=/mnt/archive`. Radarr and Sonarr describe a file by the path *they* see inside their own container, so this says what each of those prefixes is called here. **The cheapest correct value is an identity** -- mount the media share into finderr at the same place the arrs mount it, read-only, and the map is `/plex=/plex` with nothing that can drift. It is also the enumeration of every directory this process will ever open a media file from: a path that lands outside one is refused, never repaired. Empty is the default and means no media file is ever opened, so **playback does not exist until you set this**, and everything below is inert without it. The play routes are admin-only today: anonymous gets `401`, a signed-in non-admin gets the same `404` the rest of the admin surface gives |
| `FINDERR_STREAM_ENDPOINTS` | | Every address a player may stream from, besides the page's own -- a comma list, and the simple form is bare: `192.168.1.20,2001:db8:1::20,https://finderr.example.com`. Scheme, port and whether an address is local are all derived from what you wrote; `lan=`/`wan=` in front of an entry overrides the last of those for a hostname, which DNS cannot answer. Empty means the page's origin is the only route, which is what every version before this did. Setting it buys FAILOVER and LOCALITY: HLS segments are independent GETs, so the player probes the candidates, streams from whichever answers first, and moves to another when one dies -- a dead route costs one segment rather than the playback. See [Multi-homed playback](guides/multi-homed-playback.md), which also covers the Caddy, UniFi, Cloudflare and Docker networking each of them needs |
| `FINDERR_UPNP` | `false` | Ask the gateway over UPnP IGD to forward this port, and advertise the external address it reports. **Off because it opens a port on your router** -- and it buys nothing where a reverse proxy already terminates a public name. Needs host networking to reach the gateway by multicast. Every failure is a log line and never a failed stream; see the guide above |
