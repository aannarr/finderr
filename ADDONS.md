...
| [`src/plugins/tmdb.ts`](src/plugins/tmdb.ts) | The CONFIGURED one: a `secret` API key, a plain string and a default, all declared in `meta.config` and read through `c.config`. It imports nothing from core, so it is the example to copy from outside this repo -- and it shows what going dark looks like when the key is missing. |
